...
- (void)logInUsingUserID:(NSString *)theUserID password:(NSString *)thePassword  //客服端方式认证，使用SDK提供的默认界面发送微博
{
...
#pragma mark Request
//自定义请求
- (void)loadRequestWithMethodName:(NSString *)methodName    //url中除去http://api.sina.com.cn/的部分        
                       httpMethod:(NSString *)httpMethod   //GET或POST   
                           params:(NSDictionary *)params   //发送的参数集合
                     postDataType:(WBRequestPostDataType)postDataType  //Post里面的数据类型，图片，文字
...
- (void)sendWeiBoWithText:(NSString *)text image:(UIImage *)image    //通过借口发送微博
{
...
- (void)request:(WBRequest *)request didFinishLoadingWithResult:(id)result  //绑定微博成功，返回微博的信息
{
