...
static NSInteger cacheMaxCacheAge = 60*60*24*7; // 1 week (清除缓存图片的时间)
...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        DLog(@"path :%@",diskCachePath);
        if (![[NSFileManager defaultManager] fileExistsAtPath:diskCachePath])       //在应用程序沙河路径下面的Library下面的Caches下面创建一个ImageCache文件夹，用于存放缓存的图片
        {
        ...
        cacheInQueue.maxConcurrentOperationCount = 4;
        cacheOutQueue = [[NSOperationQueue alloc] init];
        cacheOutQueue.maxConcurrentOperationCount = 4;
...
- (NSString *)cachePathForKey:(NSString *)key  //把传进来的可以进行MD5加密，并拼接文件路径
{
...
- (void)notifyDelegate:(NSDictionary *)arguments   //是否从缓存中取到图片后的操作
{
    ...
    if (image)
    {
        ...
        if (delegate && [delegate respondsToSelector:@selector(imageCache:didFindImage:forKey:userInfo:)])
        {
    ...
    {
        if (delegate && [delegate respondsToSelector:@selector(imageCache:didNotFindImageForKey:userInfo:)])
        {
...
- (void)queryDiskCacheOperation:(NSDictionary *)arguments  //查询磁盘缓存操作
{
    NSString *key = [arguments objectForKey:@"key"];
    NSMutableDictionary *mutableArguments = [[arguments mutableCopy] autorelease];

    UIImage *image = [[[UIImage alloc] initWithContentsOfFile:[self cachePathForKey:key]] autorelease]; //从缓存中获取图片
    if (image)
    {
#ifdef ENABLE_SDWEBIMAGE_DECODER
...
- (UIImage *)imageFromKey:(NSString *)key   //通过key去获取image
{
    return [self imageFromKey:key fromDisk:YES];
}

- (UIImage *)imageFromKey:(NSString *)key fromDisk:(BOOL)fromDisk  //是否从本地取得图片，通过key
{
    if (key == nil)
    {
        return nil;
    ...
    if (!image && fromDisk)
    {
       // image = [[[UIImage alloc] initWithContentsOfFile:[self cachePathForKey:key]] autorelease];
        ...
        if (image)
        {
            [memCache removeAllObjects];         //永远保持缓存数组里面只有一张图片，便于节省内存空间
            [memCache setObject:image forKey:key];
        }
    }
...
- (void)queryDiskCacheForKey:(NSString *)key delegate:(id <SDImageCacheDelegate>)delegate userInfo:(NSDictionary *)info
{
    ...
    //UIImage *image = [memCache objectForKey:key];
    UIImage *image=[self imageFromKey:key];
    if (image)                  //图片已经被缓存了，不需要再去下载
    {
    ...
    // NSInvocationOperation NSInvocationOperation类是一个NSOperation管理一个单一的封装调用指定的任务执行的具体子类。你可以使用这个类开始运作，由指定的对象上调用一个选择。这个类实现了非并发操作。
